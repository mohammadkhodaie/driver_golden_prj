...
namespace pax{ namespace transport{
    ...
    public:
         ...
         */
       PAX_INLINE void commit(size_t num_bytes){
            _length = num_bytes;
        }
         ...
         */
        template <class T> PAX_INLINE T cast(void) const{
         ...
         */
        PAX_INLINE size_t size(void) const{
            return _length;
        }

        //! Create smart pointer to a reusable managed buffer
        template <typename T> PAX_INLINE boost::intrusive_ptr<T> make(
    ...
    PAX_INLINE void intrusive_ptr_add_ref(managed_buffer *p){
        ++(p->_ref_count);
    }

    PAX_INLINE void intrusive_ptr_release(managed_buffer *p){
     ...
     */
    class PAX_API managed_recv_buffer : public managed_buffer{
    public:
     ...
     */
    class PAX_API managed_send_buffer : public managed_buffer{
    public:
     ...
     */
    class PAX_API zero_copy_if : boost::noncopyable{
    public:
