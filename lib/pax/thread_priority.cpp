 ...
 **********************************************************************/
    ...
    void pax::set_thread_priority(float priority, bool realtime){
        ...
        if (min_pri == -1 or max_pri == -1) throw pax::os_error("error in sched_get_priority_min/max");

        //set the new priority and policy
        sched_param sp;
        sp.sched_priority = int(priority*(max_pri - min_pri)) + min_pri;
        int ret = pthread_setschedparam(pthread_self(), policy, &sp);
        if (ret != 0) throw pax::os_error("error in pthread_setschedparam");
    }
 ...
 **********************************************************************/
    ...
    void uhd::set_thread_priority(float, bool){
        throw pax::not_implemented_error("set thread priority not implemented");
    }
