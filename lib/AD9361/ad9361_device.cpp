...
void ad9361_device_t::set_delay(boost::int32_t clk, boost::int32_t data)
{
    boost::uint8_t rx_delays = (clk << 4)
            | (data & 0xF);
...
void ad9361_device_t::_setup_rx_fir(size_t num_taps, boost::int32_t decimation)
{
    if (not (decimation == 1 or decimation == 2 or decimation == 4)) {
        throw pax::runtime_error("[ad9361_device_t] Invalid Rx FIR decimation.");
    }
    ...
    for (size_t i = 0; i < num_taps; i++) {
        ...
        case 48:
            ...
            break;
        default:
            throw pax::runtime_error("[ad9361_device_t] Unsupported number of Rx FIR taps.");
        }
    }
...
void ad9361_device_t::_setup_tx_fir(size_t num_taps, boost::int32_t interpolation)
{
    if (not (interpolation == 1 or interpolation == 2 or interpolation == 4)) {
        throw pax::runtime_error("[ad9361_device_t] Invalid Tx FIR interpolation.");
    }
    if (interpolation == 1 and num_taps > 64) {
        throw pax::runtime_error("[ad9361_device_t] Too many Tx FIR taps for interpolation value.");
    }
    boost::scoped_array<boost::uint16_t> coeffs(new boost::uint16_t[num_taps]);
    for (size_t i = 0; i < num_taps; i++) {
        switch (num_taps) {
        ...
        case 48:
            ...
            break;
        default:
            throw pax::runtime_error("[ad9361_device_t] Unsupported number of Tx FIR taps.");
        }
    }
...
void ad9361_device_t::_calibrate_lock_bbpll()
{
    ...
    /* delete me */ std::cout << int(_io_iface->peek8(0x03f)) << std::endl;
    ...
    while (!(_io_iface->peek8(0x05e) & 0x80)) {
        if (count > 1000) {
            throw pax::runtime_error("[ad9361_device_t] BBPLL not locked");
            break;
        }
        count++;
 ...
 * initialization. */
void ad9361_device_t::_calibrate_synth_charge_pumps(direction_t direction)
{
    ...
    if ((_io_iface->peek8(0x017) & 0x0F) != 5) {
        throw pax::runtime_error("[ad9361_device_t] AD9361 not in ALERT during cal");
    }
    size_t count = 0;
    if (direction == RX) {
        /* Calibrate the RX synthesizer charge pump. */
        _io_iface->poke8(0x23d, 0x04);
        ///*SBM: enable higher length dither (17->23)*/_io_iface->poke8(0x23d, 0x44);
        while (!(_io_iface->peek8(0x244) & 0x80)) {
            if (count > 5) {
                throw pax::runtime_error("[ad9361_device_t] RX charge pump cal failure");
                break;
            }
            count++;
            boost::this_thread::sleep(boost::posix_time::milliseconds(1));
        ...
        ///*SBM: enable higher length dither (17->23)*/_io_iface->poke8(0x23d, 0x40);
    }
        ...
        while (!(_io_iface->peek8(0x284) & 0x80)) {
            if (count > 5) {
                throw pax::runtime_error("[ad9361_device_t] TX charge pump cal failure");
                break;
            }
            count++;
            boost::this_thread::sleep(boost::posix_time::milliseconds(1));
        ...
        throw pax::runtime_error("[ad9361_device_t] [_calibrate_synth_charge_pumps] INVALID_CODE_PATH");
    }
}
...
double ad9361_device_t::_calibrate_baseband_rx_analog_filter(double req_rfbw)
{
    ...
    {
        std::cout << "baseband bandwidth too large for current sample rate. Setting bandwidth to: "<<_baseband_bw;
    ...
    while (_io_iface->peek8(0x016) & 0x80) {
        if (count > 100) {
            throw pax::runtime_error("[ad9361_device_t] RX baseband filter cal FAILURE");
            break;
        }
        count++;
        boost::this_thread::sleep(boost::posix_time::milliseconds(1));
...
double ad9361_device_t::_calibrate_baseband_tx_analog_filter(double req_rfbw)
{
    ...
    while (_io_iface->peek8(0x016) & 0x40) {
        if (count > 100) {
            throw pax::runtime_error("[ad9361_device_t] TX baseband filter cal FAILURE");
            break;
        }
...
double ad9361_device_t::_calibrate_secondary_tx_filter(double req_rfbw)
{
    ...
    if ((bbbw_mhz * 2) <= 9) {
        ...
        throw pax::runtime_error("[ad9361_device_t] Cal2ndTxFil: INVALID_CODE_PATH bad bbbw_mhz");
        reg0d0 = 0x00;
    }
...
double ad9361_device_t::_calibrate_rx_TIAs(double req_rfbw)
{
    ...
    if (ceil_bbbw_mhz <= 3) {
        ...
        throw pax::runtime_error("[ad9361_device_t] CalRxTias: INVALID_CODE_PATH bad bbbw_mhz");
    }
...
void ad9361_device_t::_calibrate_baseband_dc_offset()
{
    ...
    while (_io_iface->peek8(0x016) & 0x01) {
        if (count > 100) {
            throw pax::runtime_error("[ad9361_device_t] Baseband DC Offset Calibration Failure");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_rf_dc_offset()
{
    ...
    while (_io_iface->peek8(0x016) & 0x02) {
        if (count > 100) {
            throw pax::runtime_error("[ad9361_device_t] RF DC Offset Calibration Failure");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_rx_quadrature()
{
    ...
    /* SBM */
    _io_iface->poke8(0x057, (_io_iface->peek8(0x057) & 0x30) | 0x03); // Power down Tx mixer, preserve ext. VCO buffer power down state
    ...
    while (_io_iface->peek8(0x016) & 0x20) {
        if (count > 100) {
            throw pax::runtime_error("[ad9361_device_t] Rx Quadrature Calibration Failure");
            break;
        }
        count++;
        boost::this_thread::sleep(boost::posix_time::milliseconds(5));
    ...
    /* SBM */
    _io_iface->poke8(0x057, _io_iface->peek8(0x057) & 0x30); // Re-enable Tx mixers, preserve ext. VCO buffer power down state
...
void ad9361_device_t::_tx_quadrature_cal_routine() {
    ...
    if (max_cal_freq > bbbw)
        throw pax::runtime_error("[ad9361_device_t] max_cal_freq > bbbw");
/*
...
*/
    ...
    */
    ...
    while (_io_iface->peek8(0x016) & 0x10) {
        if (count > 100) {
            throw pax::runtime_error("[ad9361_device_t] TX Quadrature Calibration Failure");
            break;
        }
        count++;
        boost::this_thread::sleep(boost::posix_time::milliseconds(10));
    }
//    std::cout<<"0x0A7: "<<std::hex<<(int)_io_iface->peek8(0x0a7)<<std::endl;
//    std::cout<<"0x0A8: "<<std::hex<<(int)_io_iface->peek8(0x0a8)<<std::endl;
}
...
void ad9361_device_t::_program_gain_table() {
    ...
    if (_rx_freq < 1300e6) {
        ...
        throw pax::runtime_error("[ad9361_device_t] Wrong _rx_freq value");
        new_gain_table = 1;
    }
 ...
 */
void ad9361_device_t::_setup_synth(direction_t direction, double vcorate, double fref)
{
    /* SBM */ // decide which table to use
#ifndef USE_UHD_TABLE
    double *vco_index = vco_index_analog;
    int (*synth_cal_lut)[53][12];
    if (fref < 50000000)
        synth_cal_lut = &synth_cal_lut_analog[0];
    else if (fref <= 70000000)
        synth_cal_lut = &synth_cal_lut_analog[1];
    else
        synth_cal_lut = &synth_cal_lut_analog[2];
#else
    double *vco_index = vco_index_uhd;
    int (*synth_cal_lut)[53][12];
     synth_cal_lut = &synth_cal_lut_uhd;
    ...
    if (vcoindex > 53)
        throw pax::runtime_error("[ad9361_device_t] vcoindex > 53");
    ...
    if (direction == RX) {
        ...
        throw pax::runtime_error("[ad9361_device_t] [_setup_synth] INVALID_CODE_PATH");
    }
}
...
{
    std::cout << boost::format("[ad9361_device_t::_tune_bbvco] rate=%.10f\n") % rate;
    ...
    if (i == 7)
        throw pax::runtime_error("[ad9361_device_t] _tune_bbvco: wrong vcorate");

    // << boost::format("[ad9361_device_t::_tune_bbvco] vcodiv=%d vcorate=%.10f\n") % vcodiv % vcorate;
...
void ad9361_device_t::_ext_lo_ctrl(direction_t direction, const lo_mode_t mode){

    /* If this function ever gets called, and the ENSM isn't already in the
     * ALERT state, then something has gone horribly wrong. */
    if ((_io_iface->peek8(0x017) & 0x0F) != 5) {
        throw pax::runtime_error("[ad9361_device_t] AD9361 not in ALERT during cal to _ext_lo_ctrl");
    }

    bool enable = (mode == EXTERNAL_LO);
    //
    /* REVIST:
     * POWER_DOWN_TRX_SYNTH and MCS_RF_ENABLE somehow conflict
     */

    bool mcs_rf_enable = !!((_io_iface->peek8(0x001)) & 0x08);

    if (direction == TX) {
        boost::uint8_t current_reg = _io_iface->peek8(0x015);
        _io_iface->poke8(0x015, (mcs_rf_enable || !enable)? current_reg & 0xdf : current_reg | 0x20);

        current_reg = _io_iface->peek8(0x005);
        _io_iface->poke8(0x005, enable? (current_reg & 0x0f) | 0x70 : current_reg);

        _io_iface->poke8(0x051, enable? 0x0e : 0x00);

        current_reg = _io_iface->peek8(0x057);
        _io_iface->poke8(0x057, enable? current_reg & 0xef : current_reg | 0x10);

        _io_iface->poke8(0x2a1, enable? 0xf0 : 0x00);
    }
    else {
        boost::uint8_t current_reg = _io_iface->peek8(0x015);
        _io_iface->poke8(0x015, (mcs_rf_enable || !enable)? current_reg & 0xbf : current_reg | 0x40);

        current_reg = _io_iface->peek8(0x005);
        _io_iface->poke8(0x005, enable? (current_reg & 0xf0) | 0x07 : current_reg);

        _io_iface->poke8(0x050, enable? 0x0e : 0x00);

        current_reg = _io_iface->peek8(0x057);
        _io_iface->poke8(0x057, enable? current_reg & 0xdf : current_reg | 0x20);

        _io_iface->poke8(0x261, enable? 0x30 : 0x00);
    }
}

void ad9361_device_t::set_rx_lo_source(lo_mode_t extint){
    if (_use_ext_rx_lo == extint)
        return;

    if (_use_ext_tx_lo == EXTERNAL_LO && extint == EXTERNAL_LO)
        std::cout<< "Warning: both Rx/Tx are set to external LO. In this mode one single LO frequency is possible." << std::endl;

    _use_ext_rx_lo = extint;

    // transition to ALERT state
    bool mcs_rf_enable = !!((_io_iface->peek8(0x001)) & 0x08); // there is a conflict with this bit
    if (extint == EXTERNAL_LO){
        _io_iface->poke8(0x015, (_io_iface->peek8(0x015)&0x20) | (mcs_rf_enable? 0x44 : 0x04)); // dual synth mode, power down rx synth, preserve tx synth power down state
        _ext_synth->powerdown(false); // enable ext. synthesizer
    }
    else{
        if(_use_ext_tx_lo==INTERNAL_LO) _ext_synth->powerdown(true); // disable ext. synthesizer
        _io_iface->poke8(0x015, (_io_iface->peek8(0x015)&0x20) | 0x04); //dual synth mode, power on rx synth, preserve tx synth power down state
    }
    _io_iface->poke8(0x014, 0x05); //use SPI for TXNRX ctrl, to ALERT
    _io_iface->poke8(0x013, 0x01); //enable ENSM
    boost::this_thread::sleep(boost::posix_time::milliseconds(1));

    _ext_lo_ctrl(RX, extint);
    boost::this_thread::sleep(boost::posix_time::milliseconds(1));

    if (_use_ext_rx_lo == INTERNAL_LO) _calibrate_synth_charge_pumps(RX);
...
void ad9361_device_t::set_tx_lo_source(lo_mode_t extint){
    if (_use_ext_tx_lo == extint)
        return;

    if (_use_ext_rx_lo == EXTERNAL_LO && extint == EXTERNAL_LO)
        std::cout<< "Warning: both Rx/Tx are set to external LO. In this mode one single LO frequency is possible." << std::endl;

    _use_ext_tx_lo = extint;

    // transition to ALERT state
    bool mcs_rf_enable = !!((_io_iface->peek8(0x001)) & 0x08); // there is a conflict with this bit
    if (extint == EXTERNAL_LO){
        _io_iface->poke8(0x015, (_io_iface->peek8(0x015)&0x40) | (mcs_rf_enable? 0x24 : 0x04)); // dual synth mode, power down tx synth, preserve rx synth power down state
        _ext_synth->powerdown(false); // enable ext. synthesizer
    }
    else{
        if(_use_ext_rx_lo==INTERNAL_LO) _ext_synth->powerdown(true); // disable ext. synthesizer
        _io_iface->poke8(0x015, (_io_iface->peek8(0x015)&0x40) | 0x04); //dual synth mode, power on tx synth, preserve rx synth power down state
    }
    _io_iface->poke8(0x014, 0x05); //use SPI for TXNRX ctrl, to ALERT
    _io_iface->poke8(0x013, 0x01); //enable ENSM
    boost::this_thread::sleep(boost::posix_time::milliseconds(1));

    _ext_lo_ctrl(TX, extint);

    if (_use_ext_tx_lo == INTERNAL_LO) _calibrate_synth_charge_pumps(TX);
...
double ad9361_device_t::_tune_helper_ext(direction_t direction, const double value)
{
    double actual_lo;

    if (direction == RX) {
        /* Set filter switches */ //SBM
//        boost::uint32_t band = get_filter_bank_sw(value);
//        if (band != get_filter_bank_sw(_req_rx_freq)){
//            _io_iface->poke32(U2_REG_SR_ADDR(SR_FILT_SW),band);
//        }
        _req_rx_freq = value;
        ...
        // no tuning is actually required for external LO
        // must do synthesizer lock here
        //_ext_synth->set_refin_freq(40e6);
        actual_lo = value; // azarbad inject
        actual_lo = _ext_synth->set_freq(value*2);
        _rx_freq = actual_lo;
...
double ad9361_device_t::_tune_helper_int(direction_t direction, const double value, bool set_filter_bank)
{
    ...
    if (i == 7)
        throw pax::runtime_error("[ad9361_device_t] RFVCO can't find valid VCO rate!");
    ...
    if (direction == RX) {
        ...
        _setup_synth(RX, actual_vcorate, fref);
...
//            throw pax::runtime_error("[ad9361_device_t] RX PLL NOT LOCKED");
//        }
...
//        /* Set band-specific settings. */
        ...
        /* Setup the synthesizer. */
        _setup_synth(TX, actual_vcorate, fref);
...
//            throw pax::runtime_error("[ad9361_device_t] TX PLL NOT LOCKED");
//        }
        uint32_t try_count = 0;
...
double ad9361_device_t::_setup_rates(const double rate)
{
    /* If we make it into this function, then we are tuning to a new rate.
      ...
      if (rate < 0.33e6) {
          ...
          throw pax::runtime_error("[ad9361_device_t] [_setup_rates] INVALID_CODE_PATH");
      }
      ...
      _regs.bbpll = _regs.bbpll | 0x10;
      _regs.bbpll = _regs.bbpll & 0x1F;
      _regs.bbpll = _regs.bbpll | 0x20;
 ...
 **********************************************************************/
void ad9361_device_t::initialize(lo_mode_t set_lo_mode)
{
    ...
    /* SBM */ // Tx uses internal lo by default
    _use_ext_rx_lo = set_lo_mode;
    _use_ext_tx_lo = INTERNAL_LO;
    ...
    /* SBM: these are fixed values that register map dictates 0x02 for them. Refer to UG-570. */
    /* They are not present in B200 driver, though. */
    _io_iface->poke8(0x024, 0x02);
    _io_iface->poke8(0x025, 0x02);
    ...
    default:
        throw pax::runtime_error("[ad9361_device_t] NOT IMPLEMENTED");
    }
    ...
    case AD9361_DDR_FDD_LVCMOS_1R_1T:
    case AD9361_DDR_FDD_LVCMOS_2R_2T:
    {
        _io_iface->poke8(0x010, 0xc8); // Swap I&Q on Tx, Swap I&Q on Rx, Toggle frame sync mode 2R2T
    ...
    default:
        throw pax::not_implemented_error("[ad9361_device_t] NOT IMPLEMENTED");
    }
    ...
    ///* SBM: enable AuxDAC & GPO manual mode */ _io_iface->poke8(0x026, 0x90); // AuxDAC Manual Select Bit/GPO Manual Select
    ...
    ///* SBM: I'm inverting this ;) */ _io_iface->poke8(0x022, 0x4A);
    ...
    ///* SBM: disable control outputs */ _io_iface->poke8(0x036, 0x00);

    ///* SBM - reduce tx gain to ensure lower third harmonic */ _io_iface->poke8(0x065, 0x01);
    /* Setup GPO */
    ...
    ///*SBM: enable higher length dither (17->23)*/_io_iface->poke8(0x23d, 0x40); // Clear half VCO cal clock setting
    ///*SBM: extra dither bits (23+n)*/_io_iface->poke8(0x241, 0xF0);
...
//    if (_use_ext_rx_lo)
//        _ext_lo_ctrl(RX, EXTERNAL_LO);
//    else
//        _ext_lo_ctrl(RX, INTERNAL_LO);
//    // TX uses internal lo by default
//    _ext_lo_ctrl(TX, INTERNAL_LO);
    ...
    switch (_client_params->get_digital_interface_mode()) {
    case AD9361_DDR_TDD_LVCMOS_1R_1T:{
...
double ad9361_device_t::set_clock_rate(const double req_rate)
{
    ...
    if (req_rate > 61.44e6) {
        throw pax::runtime_error("[ad9361_device_t] Requested master clock rate outside range");
    }
    ...
    default:
        throw pax::runtime_error("[ad9361_device_t] [set_clock_rate:1] AD9361 in unknown state");
        break;
    };
    ...
    /* SBM */
    //_io_iface->poke8(0x015, (((_use_ext_rx_lo == EXTERNAL_LO)?0x40:0x00)|(_use_ext_tx_lo == EXTERNAL_LO)?0x20:0x00)| 0x04); // preserve RX/TX synth. power down bits
    _io_iface->poke8(0x015, 0x04); //dual synth mode, synth en ctrl en
    _io_iface->poke8(0x014, 0x05); //use SPI for TXNRX ctrl, to ALERT, TX on
    _io_iface->poke8(0x013, 0x01); //enable ENSM
    boost::this_thread::sleep(boost::posix_time::milliseconds(1));
    ...
    default:
        throw pax::runtime_error("[ad9361_device_t] [set_clock_rate:2] AD9361 in unknown state");
        break;
    };
    return rate;
}
...
//        if ((_io_iface->peek8(0x017) & 0x0F) != 0x05)
//            std::cout << "ERR " << i << std::endl;
//    std::cout << "ENSM STATE: " << (int)ensm_state << std::endl;
     ...
     */
    if (tx1 | tx2){
        ///* SBM - deleteme */ _io_iface->poke8(0x0a3,1<<6); // SBM: set higher tx quad. cal. frequency
        _calibrate_tx_quadrature();
    }
...
double ad9361_device_t::tune(direction_t direction, const double value, bool set_filter_bank)
{
    ...
    if (direction == RX) {
        ...
        throw pax::runtime_error("[ad9361_device_t] [tune] INVALID_CODE_PATH");
    }
...
void ad9361_device_t::digital_test_tone(bool enb) // Digital output on TX
{
    boost::lock_guard<boost::recursive_mutex> lock(_mutex);
...
/* sbm */
void ad9361_device_t::digital_test_prbs(bool enb) // Digital output on RX
{
    boost::lock_guard<boost::recursive_mutex> lock(_mutex);
    _io_iface->poke8(0x3F4, 0xC2 | (enb ? 0x09 : 0x00));
}
...
double ad9361_device_t::_get_temperature(const double cal_offset, const double timeout)
{
    ...
    while(((_io_iface->peek8(0x00C) >> 1) & 0x01) == 0) {
        ...
        {
            throw pax::runtime_error("[ad9361_device_t] timeout while reading temperature");
        }
    }
...
void ad9361_device_t::set_dc_offset_auto(direction_t direction, const bool on)
{
    if (direction == RX) {
        ...
        throw pax::runtime_error("[ad9361_device_t] [set_dc_offset_auto] Tx DC tracking not supported");
    }
}
...
void ad9361_device_t::set_iq_balance_auto(direction_t direction, const bool on)
{
    if (direction == RX) {
        ...
        throw pax::runtime_error("[ad9361_device_t] [set_iq_balance_auto] Tx IQ tracking not supported");
    }
}
...
void ad9361_device_t::_setup_agc(chain_t chain, gain_mode_t gain_mode)
{
    ...
    {
        throw pax::runtime_error("[ad9361_device_t] Wrong value for chain");
    }
    ...
    switch (gain_mode) {
        ...
        case GAIN_MODE_FAST_AGC:
            ...
            break;
        default:
            throw pax::runtime_error("[ad9361_device_t] Gain mode does not exist");
    }
...
filter_info_base::sptr ad9361_device_t::get_filter(direction_t direction, chain_t chain, const std::string &name)
{
    if(direction == RX) {
        ...
        {
            throw pax::runtime_error("ad9361_device_t::get_filter this filter can not be read.");
        }
    ...
    throw pax::runtime_error("ad9361_device_t::get_filter wrong direction parameter.");
}
...
void ad9361_device_t::set_filter(direction_t direction, chain_t chain, const std::string &name, filter_info_base::sptr filter)
{
    ...
    if(direction == RX) {
        ...
        {
            throw pax::runtime_error("ad9361_device_t::set_filter this filter can not be written.");
        }
...
void ad9361_device_t::_set_fir_taps(direction_t direction, chain_t chain, const std::vector<boost::int16_t>& taps)
{
    ...
    if(num_taps == num_taps_avail)
    {
        ...
        throw pax::runtime_error("ad9361_device_t::_set_fir_taps not enough coefficients.");
    } else {
        throw pax::runtime_error("ad9361_device_t::_set_fir_taps too many coefficients.");
    }
}
...
std::vector<boost::int16_t> ad9361_device_t::_get_fir_taps(direction_t direction, chain_t chain)
{
    ...
    if(chain == CHAIN_1)
    {
        ...
        throw pax::runtime_error("[ad9361_device_t] Can not read both chains synchronously");
    }
...
void ad9361_device_t::ensm_force_alert()
{
    uint8_t ensm_state = _io_iface->peek8(0x017) & 0x0f;
    if (ensm_state == 0x05) {
        std::cout << "Nothing to do, device is already in alert state" << std::endl;
        return;
    }
...
void ad9361_device_t::ensm_force_fdd()
{
    uint8_t ensm_state = _io_iface->peek8(0x017) & 0x0f;
    if (ensm_state == 0xA) {
        std::cout << "Nothing to do, device is already in fdd state" << std::endl;
        return;
    }
...
void ad9361_device_t::do_mcs(boost::uint32_t step)
{
    switch (step) {
    case 0:
        /* REVIST:
         * POWER_DOWN_TRX_SYNTH and MCS_RF_ENABLE somehow conflict
         */
        _io_iface->poke8(0x015, _io_iface->peek8(0x015)&0x9F);   // clear bits POWER_DOWN_TX_SYNTH and POWER_DOWN_RX_SYNTH
        ...
        */
        ...
        _io_iface->poke8(0x001, (_io_iface->peek8(0x001)&0xF0)|0x0D);
                // set MCS_RF_ENABLE, MCS_BBPLL_ENABLE, and MCS_BB_ENABLE
                // reset MCS_DIGITAL_CLK_ENABLE
        _io_iface->poke8(0x047, 0x80);
        break;
    case 1:
        /* REVIST:
        * POWER_DOWN_TRX_SYNTH and MCS_RF_ENABLE somehow conflict
    ...
    case 2:
         ...
         */
        _io_iface->poke32(U2_REG_SR_ADDR(SR_MCS),1);
        _io_iface->poke32(U2_REG_SR_ADDR(SR_MCS),0);
//        gpio_set_value(phy->pdata->gpio_sync, 1);
//        gpio_set_value(phy->pdata->gpio_sync, 0);
        break;
    case 3:
        _io_iface->poke8(0x001, (_io_iface->peek8(0x001)&0xF0)|0x0B);
                // set MCS_RF_ENABLE, MCS_DIGITAL_CLK_ENABLE, and MCS_BB_ENABLE
                // reset MCS_BBPLL_ENABLE
        break;
    case 4:
        _io_iface->poke32(U2_REG_SR_ADDR(SR_MCS),1);
        _io_iface->poke32(U2_REG_SR_ADDR(SR_MCS),0);
...
//        gpio_set_value(phy->pdata->gpio_sync, 0);
        break;
    case 5:
        _io_iface->poke8(0x001, (_io_iface->peek8(0x001)&0xF0)|0x08);
                // set MCS_RF_ENABLE
                // reset MCS_BBPLL_ENABLE, MCS_DIGITAL_CLK_ENABLE, and MCS_BB_ENABLE
        break;
    }
}
...
/* Tune the RX or TX frequency.
 *
 * This is the publicly-accessible fast tune function. It makes sure the tune
 * isn't a redundant request, and if not, passes it on to the class's
 * internal tune function.
 *
...
void ad9361_device_t::set_gpo(uint8_t GPO_channel,bool set_valu)
{

        if(GPO_channel>3)
             throw pax::runtime_error("ad9361_ctrl got an invalid GPOchannel selected. valid selectable GPO is 0 to 3");

        uint8_t temp = _io_iface->peek8(0x26);
        temp = (temp & 0xEF) | 0x10;
        _io_iface->poke8(0x26,temp); //set GPO to manual
         uint8_t mask_temp = 0xFF & (~(1<<(GPO_channel+4)));
        temp=_io_iface->peek8(0x27)& mask_temp;
        temp= temp | ((static_cast<uint8_t>(set_valu))<<(GPO_channel+4));
        _io_iface->poke8(0x27,temp); //set manual GPO

}


void ad9361_device_t::swap_iq(direction_t direction,bool state){
    boost::uint8_t reed_back = _io_iface->peek8(0x10);
    switch (direction) {
    case RX:
        if(state)
            reed_back = (reed_back & (0xFF - 0x40));
        else
            reed_back = (reed_back | (0x00 + 0x40));
        break;
    case TX:
        if(state)
            reed_back = (reed_back & (0xFF - 0x80));
        else
            reed_back = (reed_back | (0x00 + 0x80));
        break;
    }
    _io_iface->poke8(0x10,reed_back);
}
