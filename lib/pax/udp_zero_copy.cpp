...
public:
    ...
    PAX_INLINE sptr get_new(const double timeout, size_t &index){
      ...
      // u_int16_t *gg=(u_int16_t*)_mem;

//        for(int i=0;i<(_len>>1);i++)
//            std::cout<<std::hex<<gg[i]<<std::endl;
        if (_len > 0){
        ...
        if (wait_for_recv_ready(_sock_fd, timeout)){
            ...
            PAX_ASSERT_THROW(_len > 0); // TODO: Handle case of recv error
            index++; //advances the caller's buffer
            return make(this, _mem, size_t(_len));
        }
...
public:
    ...
    void release(void){
        ...
        while (true)
        {
            ...
            PAX_ASSERT_THROW(ret == ssize_t(size()));
        }
        _claimer.release();
    }
...
template<typename Opt> static size_t resize_buff_helper(
    ...
    if (target_size > 0){
        ...
        if (actual_size < target_size) std::cout << boost::format(
...
udp_zero_copy::sptr udp_zero_copy::make(
    ...
    if (hints.has_key("recv_buff_size")) {
        if (usr_recv_buff_size < xport_params.recv_frame_size * xport_params.num_recv_frames) {
            throw pax::value_error((boost::format(
