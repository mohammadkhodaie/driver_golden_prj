...
public:
        ...
        _timeout(ACK_TIMEOUT)
    {
        ...
        this->set_time(pax::time_spec_t(0.0));
    ...
    void poke16(wb_addr_type, boost::uint16_t){
        throw pax::not_implemented_error("poke16 not implemented in fifo ctrl module");
    }

    boost::uint16_t peek16(wb_addr_type){
        throw pax::not_implemented_error("peek16 not implemented in fifo ctrl module");
    }
...
//                boost::this_thread::sleep(boost::posix_time::milliseconds(1));
                run_failed_counter++;
                std::cout<<"sfc try again"<<std::endl;
            }


        }while(run_failed_counter<3);
        if(run_failed_counter==3){
            return this->wait_for_ack(_seq_out);
        }
    ...
    // SBM, NOTE: this is wrong and for code compilation only
    boost::uint32_t transact_spi(
        int which_slave,
        const spi_config_t &config,
        ...
        //load data word (must be in upper bits)
        const boost::uint32_t data_out = data[0] << (32 - num_bits);
     ...
     ******************************************************************/
    void set_time(const pax::time_spec_t &time){
        boost::mutex::scoped_lock lock(_mutex);
        _time = time;
        _use_time = _time != pax::time_spec_t(0.0);
...
private:
     ...
     ******************************************************************/
    PAX_INLINE void send_pkt(wb_addr_type addr, boost::uint32_t data, int cmd){
        managed_send_buffer::sptr buff = _xport->get_send_buff(0.0);
        if (not buff){
            throw pax::runtime_error("fifo ctrl timed out getting a send buffer");
        }
    ...
    PAX_INLINE bool wraparound_lt16(const boost::int16_t i0, const boost::int16_t i1){
        if (((i0 ^ i1) & 0x8000) == 0) //same sign bits
            return boost::uint16_t(i0) < boost::uint16_t(i1);
        return boost::int16_t(i1 - i0) > 0;
    }

    PAX_INLINE boost::uint32_t wait_for_ack(const boost::uint16_t seq_to_ack){

        while (wraparound_lt16(_seq_ack, seq_to_ack)){
            managed_recv_buffer::sptr buff = _xport->get_recv_buff(_timeout);
            if (not buff){
                throw pax::runtime_error("fifo ctrl timed out looking for acks");
