...
namespace pax{ namespace transport{ namespace sph{

PAX_INLINE boost::uint32_t get_context_code(
    ...
    boost::uint32_t word0 = vrt_hdr[if_packet_info.num_header_words32] |
             pax::byteswap(vrt_hdr[if_packet_info.num_header_words32]);
    return word0 & 0xff;
}
...
public:
    ...
    void set_converter(const pax::convert::id_type &id){
       ...
       _converter =pax::convert::get_converter(id)();
        ...
        _bytes_per_otw_item =pax::convert::get_bytes_per_item(id.input_format);
        _bytes_per_cpu_item =pax::convert::get_bytes_per_item(id.output_format);
    }
...
//    }
     ...
     ******************************************************************/
    PAX_INLINE size_t recv(
        const pax::rx_streamer::buffs_type &buffs,
        const size_t nsamps_per_buff,
       pax::rx_metadata_t &metadata,
...
#endif
    //    for(int i=0;i<accum_num_samps;i++)
         //   std::cout<<((int*)buffs)[i]<<std::endl;
        return accum_num_samps;
    }
...
private:
   ...
   pax::convert::converter::sptr _converter; //used in conversion
     ...
     ******************************************************************/
    PAX_INLINE packet_type get_and_process_single_packet(
        const size_t index,
     ...
     ******************************************************************/
    PAX_INLINE void alignment_check(
     ...
     ******************************************************************/
    PAX_INLINE void get_aligned_buffs(double timeout){
     ...
     ******************************************************************/
    PAX_INLINE size_t recv_one_packet(
        const pax::rx_streamer::buffs_type &buffs,
        const size_t nsamps_per_buff,
       pax::rx_metadata_t &metadata,
        const double timeout,
     ...
     ******************************************************************/
	PAX_INLINE void converter_thread_task(const size_t index)
    {
        _task_barrier.wait();
    ...
    struct dbg_recv_stat_t {
        dbg_recv_stat_t(long wc, size_t nspb, size_t nsr,pax::rx_metadata_t md, double to, bool op, double rate):
    ...
    void dbg_gather_data(const size_t nsamps_per_buff, const size_t nsamps_recv,
           pax::rx_metadata_t &metadata, const double timeout,
            const bool one_packet,
