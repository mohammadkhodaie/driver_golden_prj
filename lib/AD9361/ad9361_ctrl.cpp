...
{
public:
    ad9361_io_spi(spi_wb_iface::sptr spi_iface, boost::uint32_t slave_num) :
    ...
    virtual boost::uint8_t peek8(boost::uint32_t reg)
    {
        ...
        config.mosi_edge = pax::spi_config_t::EDGE_FALL;
    ...
    virtual boost::uint8_t peek32(boost::uint32_t reg)
    {
        boost::lock_guard<boost::mutex> lock(_mutex);
        return static_cast<boost::uint8_t>(_spi_iface->peek32(reg));
    }
    ...
    // SBM, multiple spi
    virtual void poke8(std::vector<boost::uint32_t> &reg, std::vector<boost::uint8_t> &val)
    {
        PAX_ASSERT_THROW(reg.size()==val.size());
        boost::lock_guard<boost::mutex> lock(_mutex);
        ...
        std::vector<boost::uint32_t> wr_word;
        for (size_t i=0; i<reg.size(); i++)
            wr_word.push_back(AD9361_SPI_WRITE_CMD |
                           ((boost::uint32_t(reg[i]) << AD9361_SPI_ADDR_SHIFT) & AD9361_SPI_ADDR_MASK) |
                           ((boost::uint32_t(val[i]) << AD9361_SPI_DATA_SHIFT) & AD9361_SPI_DATA_MASK));
        _spi_iface->write_spi(_slave_num, config, wr_word, AD9361_SPI_NUM_BITS);
    }

    virtual void poke32(boost::uint32_t reg, boost::uint32_t val)
    {
        boost::lock_guard<boost::mutex> lock(_mutex);
        _spi_iface->poke32(reg,val);
    }
...
public:
    ...
    void set_agc_mode(const std::string &which, const std::string &mode)
    {
        ...
        if(mode == "slow") {
            ...
            throw pax::runtime_error("ad9361_ctrl got an invalid AGC option.");
        }
    }
   ...
   void  set_delay(boost::int32_t clk1, boost::int32_t data)
    {
        _device.set_delay( clk1, data);

    }
    void output_digital_test_prbs(bool enb)
    {
        _device.digital_test_prbs(enb);
    }

    void set_clk_mux_parent(bool extint)
    {
        _device.set_rx_lo_source((ad9361_device_t::lo_mode_t)extint);
    }

    void ensm_force_alert()
    {
        _device.ensm_force_alert();
    }

    void ensm_force_fdd()
    {
        _device.ensm_force_fdd();
    }

    void do_mcs(boost::uint32_t step)
    {
        _device.do_mcs(step);
    }
...
private:
    static ad9361_device_t::direction_t _get_direction_from_antenna(const std::string& antenna)
    {
        ...
        if (sub == "RX") {
            ...
            throw pax::runtime_error("ad9361_ctrl got an invalid channel string.");
        }
        return ad9361_device_t::RX;
    }
    ...
    static ad9361_device_t::chain_t _get_chain_from_antenna(const std::string& antenna)
    {
        ...
        if (sub == "1") {
            ...
            throw pax::runtime_error("ad9361_ctrl::set_gain got an invalid channel string.");
        }
        return ad9361_device_t::CHAIN_1;
    }


    void GPO_manual_set(uint8_t GPO_channel,bool set_valu){
        _device.set_gpo(GPO_channel,set_valu);
    }

    void swap_iq(const std::string &which, bool enable){
        boost::lock_guard<boost::mutex> lock(_mutex);
        ad9361_device_t::direction_t direction = _get_direction_from_antenna(which);
