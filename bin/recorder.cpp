...
recorder::sptr recorder::make(boost::shared_ptr<pax::transport::sph::recv_packet_streamer> stream, int number, int num_log)
{
    return sptr(new recorder_impl(stream, number, num_log));
}

recorder_impl::recorder_impl(boost::shared_ptr<pax::transport::sph::recv_packet_streamer> stream,int number, int num_log):
    stream(stream),number(number),num_log(num_log)
{
