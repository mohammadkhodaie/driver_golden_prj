...
std::vector<pax::transport::if_addrs_t> pax::transport::get_if_addrs(void) {
 ...
 //   UHD_ASSERT_THROW(hints.size() <= 1);
    ...
    try{
       ...
       std::cout << boost::format("Cannot open UDP transport on %s\n%s") % hint["addr"] % e.what() << std::endl;
    ...
    {
        std::cout << "USRP2 Network discovery error " << ex.what() << std::endl;
    }
    catch(...)
    {
        std::cout<< "USRP2 Network discovery unknown error " << std::endl;
    }
