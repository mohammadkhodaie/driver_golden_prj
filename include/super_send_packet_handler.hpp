...
#ifndef INCLUDED_LIBPAX_TRANSPORT_SUPER_SEND_PACKET_HANDLER_HPP
#define INCLUDED_LIBPAX_TRANSPORT_SUPER_SEND_PACKET_HANDLER_HPP
...
public:
    ...
    typedef boost::function<bool(pax::async_metadata_t &, const double)> async_receiver_type;
    ...
    void set_converter(const pax::convert::id_type &id){
        _num_inputs = id.num_inputs;
        //_converter = pax::convert::get_converter(id)();
        //this->set_scale_factor(32767.); //update after setting converter
        _bytes_per_otw_item =4; //pax::convert::get_bytes_per_item(id.output_format);
        _bytes_per_cpu_item =4;// pax::convert::get_bytes_per_item(id.input_format);
    }
    ...
    bool recv_async_msg(
        pax::async_metadata_t &async_metadata, double timeout = 0.1
    ){
     ...
     ******************************************************************/
    PAX_INLINE size_t send(
        const pax::tx_streamer::buffs_type &buffs,
        const size_t nsamps_per_buff,
        const pax::tx_metadata_t &metadata,
...
private:
    ...
    pax::convert::converter::sptr _converter; //used in conversion
    ...
    pax::tx_metadata_t _metadata_cache;

#ifdef PAX_TXRX_DEBUG_PRINTS
    struct dbg_send_stat_t {
        dbg_send_stat_t(long wc, size_t nspb, size_t nss, pax::tx_metadata_t md, double to, double rate):
            wallclock(wc), nsamps_per_buff(nspb), nsamps_sent(nss), metadata(md), timeout(to), samp_rate(rate)
        {}
        long wallclock;
        size_t nsamps_per_buff;
        size_t nsamps_sent;
        pax::tx_metadata_t metadata;
        double timeout;
        double samp_rate;
    ...
    void dbg_print_send(size_t nsamps_per_buff, size_t nsamps_sent,
            const pax::tx_metadata_t &metadata, const double timeout,
     ...
     ******************************************************************/
    PAX_INLINE size_t send_one_packet(
        const pax::tx_streamer::buffs_type &buffs,
        const size_t nsamps_per_buff,
        vrt::if_packet_info_t &if_packet_info,
        const double timeout,
     ...
     */
    PAX_INLINE void convert_to_in_buff(const size_t index)
    {
     ...
     //   _converter->conv(in_buffs, otw_mem, _convert_nsamps);
        memcpy((void*)otw_mem,(void*)in_buffs[0],_convert_nsamps*4);
...
#endif /* INCLUDED_LIBPAX_TRANSPORT_SUPER_SEND_PACKET_HANDLER_HPP */
