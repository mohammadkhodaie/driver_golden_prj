 ...
 **********************************************************************/
typedef pax::dict<convert::id_type, pax::dict<convert::priority_type, convert::function_type> > fcn_table_type;
...
void pax::convert::register_converter(
    ...
    //----------------------------------------------------------------//
   // std::cout << "register_converter: " << id.to_pp_string() << std::endl
    //    << "    prio: " << prio << std::endl
    //    << std::endl
  //  ;
    //----------------------------------------------------------------//
...
convert::function_type convert::get_converter(
    ...
    if (not get_table().has_key(id)) throw pax::key_error(
...
size_t convert::get_bytes_per_item(const std::string &format){
    ...
    throw pax::key_error("Cannot find an item size:\n" + format);
}
