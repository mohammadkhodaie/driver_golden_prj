...
#ifndef INCLUDED_PAX_TRANSPORT_BOUNDED_BUFFER_IPP
#define INCLUDED_PAX_TRANSPORT_BOUNDED_BUFFER_IPP
...
namespace pax{ namespace transport{
    ...
    public:
        ...
        PAX_INLINE bool push_with_haste(const elem_type &elem){
        ...
        PAX_INLINE bool push_with_pop_on_full(const elem_type &elem){
            boost::mutex::scoped_lock lock(_mutex);
            if (_buffer.full()){
                _buffer.pop_back();
                _buffer.push_front(elem);
        ...
        PAX_INLINE void push_with_wait(const elem_type &elem){
        ...
        PAX_INLINE bool push_with_timed_wait(const elem_type &elem, double timeout){
        ...
        PAX_INLINE bool pop_with_haste(elem_type &elem){
            boost::mutex::scoped_lock lock(_mutex);
        ...
        PAX_INLINE void pop_with_wait(elem_type &elem){
        ...
        PAX_INLINE bool pop_with_timed_wait(elem_type &elem, double timeout){
    ...
    private:
         ...
         */
        PAX_INLINE void pop_back(elem_type &elem){
            elem = _buffer.back();
            _buffer.back() = elem_type();
            _buffer.pop_back();
        }

        static PAX_INLINE boost::posix_time::time_duration to_time_dur(double timeout){
...
#endif /* INCLUDED_PAX_TRANSPORT_BOUNDED_BUFFER_IPP */
