...
namespace pax{ namespace convert{

    //! A conversion class that implements a conversion from inputs -> outputs.
    class converter{
    public:
        typedef boost::shared_ptr<converter> sptr;
        typedef pax::ref_vector<void *> output_type;
        typedef pax::ref_vector<const void *> input_type;
        ...
        PAX_INLINE void conv(const input_type &in, const output_type &out, const size_t num){
    ...
    PAX_API bool operator==(const id_type &, const id_type &);
     ...
     */
    PAX_API void register_converter(
        const id_type &id,
     ...
     */
    PAX_API function_type get_converter(
        const id_type &id,
     ...
     */
    PAX_API void register_bytes_per_item(
        const std::string &format, const size_t size
    );

    //! Convert an item format to a size in bytes
    PAX_API size_t get_bytes_per_item(const std::string &format);
