...
void get_freq_and_freq_word(
    ...
    PAX_ASSERT_THROW(std::abs(freq) <= tick_rate/2.0);
...
public:
    ...
    void set_underflow_policy(const std::string &policy){
        ...
        else throw pax::value_error("PAX TX cannot handle requested underflow policy: " + policy);
    }
    ...
    pax::meta_range_t get_host_rates(void){
        meta_range_t range;
    ...
    pax::meta_range_t get_freq_range(void){
        return pax::meta_range_t(-_tick_rate/2, +_tick_rate/2, _tick_rate/std::pow(2.0, 32));
    }
    ...
    void setup(const pax::stream_args_t &stream_args){
        ...
        else throw pax::value_error("USRP TX cannot handle requested wire format: " + stream_args.otw_format);
