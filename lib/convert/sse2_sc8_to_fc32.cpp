...
PAX_INLINE void unpack_sc32_4x(
    const __m128i &in,
...
DECLARE_CONVERTER(sc8_item32_be, 1, fc32, 1, PRIORITY_SIMD){
    ...
    if ((size_t(inputs[0]) & 0x3) != 0){
        item32_sc8_to_xx<pax::ntohx>(input++, output++, 1, scale_factor);
        num_samps--;
    }
    ...
    //convert remainder
    item32_sc8_to_xx<pax::ntohx>(input+i, output+j, num_samps-j, scale_factor);
}
...
DECLARE_CONVERTER(sc8_item32_le, 1, fc32, 1, PRIORITY_SIMD){
    ...
    if ((size_t(inputs[0]) & 0x3) != 0){
        item32_sc8_to_xx<pax::wtohx>(input++, output++, 1, scale_factor);
        num_samps--;
    }
    ...
    //convert remainder
    item32_sc8_to_xx<pax::wtohx>(input+i, output+j, num_samps-j, scale_factor);
}
